...
// MODELOS PARA ANIMACION POR KEYFRAMES
Model balon;
Model cuerpoReptile;
Model piernaReptile01;
Model piernaReptile02;
...
//NEW// Keyframes
float posXBalon = -122.0, posYBalon = -0.6, posZBalon = 102.5;
float	movBalon_z = 0.0f, movBalon_y = 0.0f;
float giroBalon = 0;
float giroPierna01 = 0, giroPierna02 = 0;
...
typedef struct _frame
{
	...
	float movBalon_z;		//Variable para PosicionX
	float movBalon_y;		//Variable para PosicionY
	float movBalon_zInc;		//Variable para IncrementoX
	float movBalon_yInc;		//Variable para IncrementoY
	float giroBalon;
	float giroBalonInc;
	float giroPierna01;
	float giroPierna01Inc;
	float giroPierna02;
	float giroPierna02Inc;
}FRAME;
...
void saveFrame(void) //tecla L
{
	...
	printf("HOLA -  frameindex %d\n", FrameIndex);


	KeyFrame[FrameIndex].movBalon_z = movBalon_z;
	KeyFrame[FrameIndex].movBalon_y = movBalon_y;
	KeyFrame[FrameIndex].giroBalon = giroBalon;
	KeyFrame[FrameIndex].giroPierna01 = giroPierna01;
	KeyFrame[FrameIndex].giroPierna02 = giroPierna02;


	// Guardar los valores de movBalon_z, movBalon_y y giroBalon en un archivo de texto
	std::ofstream archivo("P10-318222327.txt", std::ios::app); // `app` para añadir sin sobrescribir
	if (archivo.is_open()) {
		archivo << "KeyFrame[" << FrameIndex << "].movBalon_z = " << movBalon_z << ".0f;\n";
		archivo << "KeyFrame[" << FrameIndex << "].movBalon_y = " << movBalon_y << ".0f;\n";
		archivo << "KeyFrame[" << FrameIndex << "].giroBalon = " << giroBalon << ".0f;\n";
		archivo << "KeyFrame[" << FrameIndex << "].giroPierna01 = " << giroPierna01 << ".0f;\n";
		archivo << "KeyFrame[" << FrameIndex << "].giroPierna02 = " << giroPierna02 << ".0f;\n\n";


		archivo.close();
		std::cout << "Frame guardado en 'P10-318222327.txt'.\n";
	}
	else {
		std::cerr << "No se pudo abrir el archivo para guardar el frame.\n";
	}
...
void resetElements(void) //Tecla 0
{
	...
	movBalon_z = KeyFrame[0].movBalon_z;
	movBalon_y = KeyFrame[0].movBalon_y;
	giroBalon = KeyFrame[0].giroBalon;
	giroPierna01 = KeyFrame[0].giroPierna01;
	giroPierna02 = KeyFrame[0].giroPierna02;
}

void interpolation(void)
{
	KeyFrame[playIndex].movBalon_zInc = (KeyFrame[playIndex + 1].movBalon_z - KeyFrame[playIndex].movBalon_z) / i_max_steps;
	...
	KeyFrame[playIndex].giroPierna01Inc = (KeyFrame[playIndex + 1].giroPierna01 - KeyFrame[playIndex].giroPierna01) / i_max_steps;
	KeyFrame[playIndex].giroPierna02Inc = (KeyFrame[playIndex + 1].giroPierna02 - KeyFrame[playIndex].giroPierna02) / i_max_steps;
}
...
void animate(void)
{
	...
	if (play)
	{
		...
		{
			//Dibujar Animación
			movBalon_z += KeyFrame[playIndex].movBalon_zInc;
			movBalon_y += KeyFrame[playIndex].movBalon_yInc;
			giroBalon += KeyFrame[playIndex].giroBalonInc;
			giroPierna01 += KeyFrame[playIndex].giroPierna01Inc;
			giroPierna02 += KeyFrame[playIndex].giroPierna02Inc;
			i_curr_steps++;
		}
...
int main()
{
	...
	// MODELOS PARA ANIMACIONES POR KEYFRAMES
	balon = Model();
	balon.LoadModel("Models/balon.obj");
	cuerpoReptile = Model();
	cuerpoReptile.LoadModel("Models/reptileCuerpo.obj");
	piernaReptile01 = Model();
	piernaReptile01.LoadModel("Models/reptilePierna01.obj");
	piernaReptile02 = Model();
	piernaReptile02.LoadModel("Models/reptilePierna02.obj");
	...
	glm::vec3 posBalon = glm::vec3(-122.0f, -0.6f, 120.5f);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// MATRIZ AUXILIAR PARA EL MODELO POR KEYFRAMES
		glm::mat4 modelauxReptileCuerpo(1.0);
		glm::mat4 modelauxPiernaReptile(1.0);
		...
		// BALON
		model = glm::mat4(1.0);
		posBalon = glm::vec3(posXBalon, posYBalon + movBalon_y, posZBalon + movBalon_z);
		model = glm::translate(model, posBalon);
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		model = glm::rotate(model, giroBalon * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		...
		// REPTILE CUERPO
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-120.0f, 8.0f, 100.0));
		
		modelauxReptileCuerpo = model; // GUARDA

		model = glm::scale(model, glm::vec3(8.0f, 8.0f, 8.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		cuerpoReptile.RenderModel();

		model = modelauxReptileCuerpo; // A PATIR DEL CUERPO

		model = glm::translate(model, glm::vec3(-0.5f, -0.8f, 0.0));
		model = glm::rotate(model, giroPierna01 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));

		modelauxPiernaReptile = model; // GUARDA

		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		esfera.RenderModel();

		model = modelauxPiernaReptile; // A PARTIR DE LA PIERNA

		model = glm::translate(model, glm::vec3(-0.3f, 0.48f, 0.0));

		modelauxPiernaReptile = model; // GUARDA

		model = glm::scale(model, glm::vec3(8.0f, 8.0f, 8.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		piernaReptile01.RenderModel();

		model = modelauxPiernaReptile; // A PARTIR DE LA PIERNA

		model = glm::translate(model, glm::vec3(-0.5f, -5.0f, 0.0));
		model = glm::rotate(model, giroPierna02 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_L])
	{
		...
		{
			saveFrame();
			printf("movBalon_z es: %f\n", movBalon_z);
			printf("movBalon_y es: %f\n", movBalon_y);
			printf("giroBalon es: %f\n", giroBalon);
			printf("giroPierna01 es: %f\n", giroPierna01);
			printf("giroPierna02 es: %f\n", giroPierna02);
